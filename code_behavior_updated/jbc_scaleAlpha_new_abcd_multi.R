...
jbc_scaleAlpha_new_abcd_multi<- function(X, Y,W, H, D=2,theoretical.str,family_behavior,
...
#   colnames(TAC) <- names_i
  ...
  for (s in 1:(nscan + burn))
  {
    ...
    U<-rU_b_updated_abcd_3(Fl,EFl,U,Theta, Stheta, Sutheta, Su,  s2)
  ...
  fit <- list(BETAPM=BETAPM,GAMMAPM=GAMMAPM, COV=COV, APM=APM,bPM=bPM,U=U,UVPM.l=UVPM.l,THETAPM = THETAPM,UPM=UPM, ALPHAPM = ALPHAPM,EFlPM=EFlPM,ETPM=ETPM,
              X=X,Y=Y, Theta=Theta, Alpha=Alpha, b=b, input=input, s=s, FLPM=FLPM, TMPM=TMPM)
